...
function App() {
  ...
  useEffect(() => {
    const syncWithDB = async () => {
      ...
      setHasLoaded(false); // Reset loading state when fetching new user data
  ...
  const handleImportPDF = (e) => {
    ...
    const processLog = (importedLog) => {
      if (window.confirm(`Found data for ${importedLog.monthYear}. Import this log?`)) {
         const exists = logs.find(l => l.id === importedLog.id || l.monthYear === importedLog.monthYear);
         if (exists) {
            if (window.confirm('This log month already exists. Overwrite?')) {
               setLogs(logs.map(l => (l.id === importedLog.id || l.monthYear === importedLog.monthYear) ? importedLog : l));
               setActiveLogId(importedLog.id);
            }
         } else {
            setLogs([importedLog, ...logs]);
            setActiveLogId(importedLog.id);
            if (!availableYears.includes(importedLog.taxYear)) {
               setSelectedYear(importedLog.taxYear);
            }
         }
    ...
    reader.onload = async (evt) => {
      ...
      const match = content.match(/\/Keywords\s*\(([A-Za-z0-9+/=]+)\)/);
      if (match && match[1]) {
        try {
          const jsonStr = decodeURIComponent(atob(match[1]));
          const importedLog = JSON.parse(jsonStr);
          processLog(importedLog);
        } catch (err) {
          alert("Failed to parse embedded backup data.");
        }
      } else {
        if (window.confirm("This PDF doesn't have embedded backup data. Attempt to extract visual text? (Some precision/emojis may be lost)")) {
          try {
            const legacyLog = await parseLegacyPDF(file);
            processLog(legacyLog);
          } catch (err) {
            alert("Legacy extraction failed: " + err.message);
          }
        }
      }
        ...
        {activeTab === 'settings' && (
          <div className="settings-view">
            ...
            <div className="graphs-grid" style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginTop: '1rem'}}>
              ...
              {/* Bento Card for Backup & Restore */}
              <div className="graph-card brutalist-card settings-card" style={{padding: '2rem'}}>
                <h3 style={{marginBottom: '1rem'}}>Backup & Restore</h3>
                <p style={{color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: '1.6'}}>
                  PDFs generated by FreeLog now contain a hidden backup of your data. If you ever lose your data, simply upload a generated PDF to restore that month's log instantly!
                </p>
                <div style={{display: 'flex', flexDirection: 'column', gap: '1rem'}}>
                  <label className="btn-primary" style={{textAlign: 'center', cursor: 'pointer', display: 'block', margin: 0}}>
                    RESTORE FROM PDF
